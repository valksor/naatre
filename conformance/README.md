...
Fetch worker adapter. Issue #101 supplies separate Node, Bun, Deno, and edge
package subpaths with ready/draining/stopped lifecycle, shutdown cancellation,
and stable redacted admission failures without redefining #60. Every runtime
harness executes registration, prototype-safety, AbortSignal, streaming,
backpressure, warm isolation, output validation, shutdown, safe-failure, and
resource-limit vectors. Node and Bun results are recorded independently; the
Deno and pinned workerd commands execute the identical required vector list in
integration. The combined publication matrix remains owned by #69.
