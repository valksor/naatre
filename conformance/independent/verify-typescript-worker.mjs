...
for (const runtime of fixture.runtimes) {
  ...
  assert.ok(["passed", "deferred-to-integration"].includes(runtime.execution));
...
for (const evidence of [...fixture.schema.sources, ...fixture.evidence]) {
...
validateRuntime(node.stdout, "node-26.8.2", ["plain-javascript", "typescript"]);
const bun = run("bun", [file("conformance/independent/typescript-worker-runtime.mjs")], "Bun worker runtime");
validateRuntime(bun.stdout, "bun-1.4.2", ["plain-javascript", "typescript"]);

function validateRuntime(output, runtime, implementations) {
  ...
  for (const implementation of result.implementations) assert.deepEqual(implementation.vectors, fixture.handlerImplementations[0].vectors);
}
...
process.stdout.write(`${JSON.stringify({ profile: fixture.profile, status: "passed", runtimes: ["node", "bun"], deferred: ["deno", "workerd"] })}\n`);
