...
runtime_log=$(mktemp /tmp/naatre-adapter-workerd-log.XXXXXX)
result_file=$(mktemp /tmp/naatre-adapter-workerd-result.XXXXXX)
runtime_pid=""
cleanup() {
...
if [ -n "${WORKERD_BIN:-}" ]; then
  ...
  "$WORKERD_BIN" serve conformance/edge/typescript-adapter-workerd.capnp >"$runtime_log" 2>&1 &
else
  npx --yes --userconfig=/dev/null workerd@1.20260914.1 serve \
    conformance/edge/typescript-adapter-workerd.capnp >"$runtime_log" 2>&1 &
fi
runtime_pid=$!
runtime_port=""
attempt=0
...
node --input-type=module -e '
  ...
  if (result.profile !== "sdk.typescript.adapters-1" || result.status !== "passed" || result.runtime !== "edge-workerd-2026-09-14") process.exit(1);
' "$result_file"
cat "$result_file"
