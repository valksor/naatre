...
func assertNormativeCoverage(t *testing.T, manifest suiteManifest) {
	...
	if len(coveredClauses) != 643 {
		t.Errorf("mapped normative clauses = %d, want 643", len(coveredClauses))
	}
	if len(mustClauses) != 304 {
		t.Errorf("mapped MUST/MUST NOT clause blocks = %d, want 304", len(mustClauses))
	}
