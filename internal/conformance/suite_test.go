...
func assertNormativeCoverage(t *testing.T, manifest suiteManifest) {
	...
	if len(coveredClauses) != 634 {
		t.Errorf("mapped normative clauses = %d, want 634", len(coveredClauses))
	}
	if len(mustClauses) != 309 {
		t.Errorf("mapped MUST/MUST NOT clause blocks = %d, want 309", len(mustClauses))
	}
