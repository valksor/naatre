...
func assertNormativeCoverage(t *testing.T, manifest suiteManifest) {
	...
	if len(coveredClauses) != 636 {
		t.Errorf("mapped normative clauses = %d, want 636", len(coveredClauses))
	}
	if len(mustClauses) != 313 {
		t.Errorf("mapped MUST/MUST NOT clause blocks = %d, want 313", len(mustClauses))
	}
