...
func assertNormativeCoverage(t *testing.T, manifest suiteManifest) {
	...
	if len(coveredClauses) != 638 {
		t.Errorf("mapped normative clauses = %d, want 638", len(coveredClauses))
	}
	if len(mustClauses) != 315 {
		t.Errorf("mapped MUST/MUST NOT clause blocks = %d, want 315", len(mustClauses))
	}
