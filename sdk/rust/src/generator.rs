...
use std::collections::BTreeMap;\n\n",
    ...
    for binding in operations {
        ...
        write_server_handler(&mut source, binding)?;
    }
    write_manifest_function(&mut source, operations)?;
    Ok(source)
}

fn write_server_handler(
    source: &mut String,
    binding: &OperationBinding<'_>,
) -> Result<(), ClientError> {
    let operation = rust_type_name(&binding.model.name)?;
    let variables = format!("{operation}Variables");
    let result = format!("{operation}Result");
    let method = format!("handle_{}", rust_field_name(&binding.model.name)?);
    let register = format!("register_{}_handler", rust_field_name(&binding.model.name)?);
    let trait_name = format!("{operation}Handler");
    let effect = match binding.kind {
    ...
    writeln!(
        ...
        "#[cfg(feature = \"server\")]\n\
         ...
         /// [`{register}`] and advertise the exact descriptor explicitly.\n\
         pub trait {trait_name}: Send + Sync {{\n\
             fn {method}<'a>(\n\
                 &'a self,\n\
                 context: &'a mut HandlerContext,\n\
                 input: {variables},\n\
             ) -> HandlerFuture<'a, {result}>;\n\
         }}\n\n\
         #[cfg(feature = \"server\")]\n\
         ...
         pub fn {register}<H>(builder: &mut ServerBuilder, handler: H) -> Result<(), ServerError>\n\
         where\n\
             H: {trait_name} + 'static,\n\
         {{\n\
             builder.register_handler(\n\
                 HandlerDescriptor::unary({:?}, {:?}, {:?}, {effect:?}),\n\
                 handler,\n\
                 |handler, context, input| handler.{method}(context, input),\n\
             )\n\
         }}\n",
        binding.model.name, variables, result
    )
