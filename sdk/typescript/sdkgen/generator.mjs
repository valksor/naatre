...
function generateSource(model, operations, mappings) {
  const runtimeImports = new Set(["createOperation", "createScalarCodecs", "decodeSelected", "defineHandler", "safeObject"]);
...
function operationDeclaration(operation, mappings) {
  ...
  lines.push(...serverDeclaration(operation, mappings));
  const definition = {
    name: operation.name,
    kind: operation.kind,
...
function serverDeclaration(operation, mappings) {
  const name = `${operation.symbol}HandlerOutput`;
  const declarations = [];
  const output = renderServerValue(operation.result, name, mappings, declarations);
  const inputFields = [...operation.variables].sort(compareName).map((variable) => ({
    name: variable.name,
    schema: { kind: "scalar", type: variable.type },
    required: Boolean(variable.required),
    nullable: Boolean(variable.nullable),
  }));
  const definition = {
    id: operation.name,
    inputSchema: `${operation.symbol}Variables`,
    outputSchema: name,
    codec: "naatre.json-1",
    effect: operation.kind,
    requiredCapabilities: [],
    executionProfile: "inline",
    input: { kind: "object", fields: inputFields, additionalProperties: false },
    output: output.schema,
  };
  const lines = [...declarations];
  if (operation.result.kind === "object") lines.push(`export interface ${name} {`, ...output.interfaceFields, "}", "");
  else lines.push(`export type ${name} = ${output.type};`, "");
  lines.push(
    `export type ${operation.symbol}Handler = Handler<${operation.symbol}Variables, ${name}>;`,
    "",
    `const ${lowerFirst(operation.symbol)}HandlerDefinition = ${formatValue(definition)} as const satisfies HandlerDefinition<${operation.symbol}Variables, ${name}>;`,
    "",
    `export function create${operation.symbol}Handler(handler: ${operation.symbol}Handler, customCodecs: ScalarCodecs = {}): DefinedHandler<${operation.symbol}Variables, ${name}> {`,
    `  return defineHandler({ ...${lowerFirst(operation.symbol)}HandlerDefinition, codecs: createScalarCodecs(customCodecs) }, handler);`,
    "}",
    "",
  );
  return lines;
...
function renderServerValue(node, name, mappings, declarations) {
  if (node.kind === "scalar") return { type: scalarType(node.type, mappings), schema: { kind: "scalar", type: node.type } };
  if (node.kind === "list" && node.element) {
    const element = renderServerValue(node.element, `${name}Element`, mappings, declarations);
    return { type: `readonly (${element.type}${node.element.nullable ? " | null" : ""})[]`, schema: { kind: "list", element: element.schema, elementNullable: Boolean(node.element.nullable) } };
  }
  ...
  for (const field of node.fields) {
    ...
    const child = renderServerValue(field.result, nestedName, mappings, declarations);
    ...
    interfaceFields.push(`  readonly ${propertyName(field.name)}${required ? "" : "?"}: ${child.type}${field.result.nullable ? " | null" : ""};`);
    schemaFields.push({ name: field.name, schema: child.schema, required, nullable: Boolean(field.result.nullable) });
  }
  return { type: name, interfaceFields, schema: { kind: "object", fields: schemaFields, additionalProperties: false } };
}
